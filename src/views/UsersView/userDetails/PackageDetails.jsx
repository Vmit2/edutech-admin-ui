...
import { usePackageState } from "../../../hooks/redux/usePackageState";
...
function PackageDetails(props) {
  ...
  const { details } = usePackageState();
  const detail = details; //formatePageaDeatail(details);
  return (
    <DashboardPage >
      <Grid container className={classes.packageDetailsWrapper}>
        <Grid item container>
          ...
          <Grid container>
            ...
            <Grid item xs={12} sm={6} md={6}>
              <LabelValue label="BoardId:" value={detail.boardId} />
            </Grid>
            <Grid item xs={12} sm={6} md={6}>
              <LabelValue label="Standard Id:" value={detail.standardId} />
            </Grid>
            <Grid item xs={12} sm={6} md={6}>
            ...
            <Grid item xs={12} sm={6} md={6}>
              <LabelValue
                label="Purchase Date "
                value={getformatedDate(detail.purchaseDate)}
              />
            </Grid>
            <Grid item xs={12} sm={6} md={6}>
              <LabelValue
                label="Expiry Date: "
                value={getformatedDate(detail.expiryDate)}
              />
            </Grid>
          </Grid>
        </Grid>
        {detail.subjectDetails && (
          <Grid item container>
            <Typography variant="h4" className={classes.moreDetailsTitle}>
              Subject Details:
            </Typography>
            <Grid container>
              ...
              <Grid item xs={12} sm={12} md={12}>
                <LabelValue label="Subject List:" />
                <ul>
                  {detail.subjectDetails.subjectList &&
